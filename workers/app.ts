    ...
    const orders = (
      ...
      .map((session: any) => {
        ...
        <Order>
          ...
          <OrderDate>${shipStationDate(session.created)}</OrderDate>
          <OrderStatus>${xmlValue("paid")}</OrderStatus>
          <LastModified>${shipStationDate(session.created)}</LastModified>
          <ShippingMethod>${xmlValue("Standard Shipping")}</ShippingMethod>
          <PaymentMethod>${xmlValue("Credit Card")}</PaymentMethod>
          <CurrencyCode>USD</CurrencyCode>
          <OrderTotal>${stripeMoney(session.amount_total)}</OrderTotal>
          ...
          <Customer>
            <CustomerCode>${xmlValue(email || orderNumber)}</CustomerCode>
            <BillTo>
              <Name>${xmlValue(customer.name ?? name)}</Name>
              <Company></Company>
              <Phone>${xmlValue(customer.phone ?? "")}</Phone>
              <Email>${xmlValue(email)}</Email>
            </BillTo>
            <ShipTo>
              <Name>${xmlValue(name)}</Name>
              <Company>${xmlValue(shipping.company ?? "")}</Company>
              <Address1>${xmlValue(address.line1)}</Address1>
              <Address2>${xmlValue(address.line2)}</Address2>
              <City>${xmlValue(address.city)}</City>
              <State>${xmlValue(address.state)}</State>
              <PostalCode>${xmlValue(address.postal_code)}</PostalCode>
              <Country>${xmlValue(address.country ?? "US")}</Country>
              <Phone>${xmlValue(customer.phone ?? "")}</Phone>
            </ShipTo>
          </Customer>
          <Items>${items}</Items>
